 ...
 * by deleting the table, so every reload emptied every dynamic alias and the
...
$out[] = '#!/usr/sbin/nft -f';
/*
 * Hand the traffic of the monitored interfaces to suricata when the intrusion
 * detection runs inline.
 *
 * MurOS: FreeBSD had two ways to put suricata in the path, netmap and an ipfw
 * divert socket, both before pf. Linux has the netfilter queue, so the ruleset
 * itself does the handover, in chains evaluated before the firewall chains so
 * inspection does not depend on the policy. suricata runs in repeat mode: a
 * packet it lets through comes back marked, and the first rule of each chain
...
{
    $ids = $cfg->OPNsense->IDS->general ?? null;
    if ($ids === null || empty((string)$ids->enabled)) {
        return [];
    }

    /* pcap and afpacket only listen, netmap and divert are the old spellings of
     * an inline setup and are read as the queue */
    if (!in_array((string)$ids->mode, ['nfqueue', 'netmap', 'divert'])) {
        return [];
    }

    $devs = [];
    foreach (explode(',', (string)$ids->interfaces) as $ik) {
        $devs = array_merge($devs, iface_devices($ifaces, $ik));
    }
    $devs = array_values(array_unique(array_filter($devs)));
    if (empty($devs)) {
        return [];
    }

    $queues = (int)((string)$ids->divert_listeners ?: 1);
    if ($queues < 1) {
        $queues = 1;
    }
    $target = $queues > 1 ? sprintf('0-%d', $queues - 1) : '0';
    $handover = sprintf('queue flags bypass to %s', $target);
    $ifexpr = ifname_expr($devs);
...
/* Replace our own table and nothing else. A "flush ruleset" would also take
 * away the tables other parts of the system own, first among them the captive
 * portal, whose authenticated clients and traffic counters live in
 * "inet captiveportal": every firewall reload used to disconnect them and
 * reset their accounting. The add/delete pair makes the removal idempotent
 * and the whole file still loads as one transaction. */
$out[] = 'table inet muros';
$out[] = 'delete table inet muros';
$out[] = '';
$out[] = 'table inet muros {';
...
$out[] = '    }';
$out[] = '';
foreach (ids_lines($cfg, $ifaces) as $line) {
    $out[] = $line;
}
if (!empty($nat['pre'])) {
